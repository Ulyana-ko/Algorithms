...
void c_sorttree(int * mas, int j, int n){
    if (2*j + 1 > n)
        return;
    if (2*j + 1 == n){
        if (mas[j] < mas[2*j + 1]){
            ...
            mas[j] = mas[2*j + 1];
            mas[2*j + 1] = c;
            c_sorttree(mas, 2*j+1, n);
        }
        return;
    }
    if (2*j + 1 < n){
        c_sorttree(mas, 2*j + 1, n);
        c_sorttree(mas, 2*j + 2, n);
        if (mas[j] < mas[2*j + 1]){
            int c = mas[j];
            mas[j] = mas[2*j + 1];
            mas[2*j + 1] = c;
...
void introsort(int * mas, int l, int r, int tmp, int length, int ind){
    ...
    tmp++;
    ind = 0;
    introsort(mas, m, l, tmp, length, ind);
    if (ind != 1){
        if (m < l)
            psort(mas + m, 0, l-m);
    }
    introsort(mas, r, n, tmp, length, ind);
    if (ind != 1){
        if (r < n)
            psort(mas + r, 0, n-r);
    }
